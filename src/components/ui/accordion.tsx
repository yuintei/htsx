  ...
  return (
    ...
    >
      {items.map((item) => (
        ...
        >
          <summary class="flex cursor-pointer list-none items-center justify-between gap-4 px-3 py-2 font-medium select-none hover:opacity-80">
            {item.title}
            <span class="shrink-0 text-muted-foreground transition-transform group-open/accordion-item:rotate-180">
              <ChevronDown />
            </span>
          </summary>
          <div class="px-3 pb-4 text-muted-foreground">{item.content}</div>
        </details>
      ))}
    </div>
  );
